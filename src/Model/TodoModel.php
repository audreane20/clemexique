...
class TodoModel
{
    ...
    private function resolveDestinationCategoryId(string $language, array $data): int
    {
        ...
        if ($categoryChoice === '__new__') {
            ...
            $stmt = $this->pdo->prepare(
                'INSERT INTO todo_categories (language_code, title, icon_code, sort_order)
    ...
    {
        $stmt = $this->pdo->prepare(
            'INSERT INTO todo_items (category_id, name, address, note, website_url, website_label, video_url, sort_order)
             VALUES (:category_id, :name, :address, :note, :website_url, :website_label, :video_url, :sort_order)'
        );
    ...
    private function deleteItemInLanguage(string $language, int $categoryIndex, int $itemIndex, bool $strict): void
    {
        ...
        $stmt = $this->pdo->prepare('DELETE FROM todo_items WHERE id = :id');
        $stmt->execute([
            'id' => (int) $itemRow['id'],
        ]);
    ...
    private function deleteCategoryInLanguage(string $language, int $categoryIndex, bool $strict): void
    {
        ...
        $stmt = $this->pdo->prepare('DELETE FROM todo_categories WHERE id = :id');
        $stmt->execute([
            'id' => (int) $categoryRow['id'],
        ]);
    ...
    {
        $stmt = $this->pdo->prepare(
            'SELECT * FROM todo_items WHERE category_id = :category_id ORDER BY sort_order ASC, id ASC'
        );
        $stmt->execute([
            'category_id' => $categoryId,
        ]);
    ...
    {
        $stmt = $this->pdo->prepare(
            'UPDATE todo_categories SET sort_order = :sort_order WHERE id = :id'
        );
    ...
    {
        $stmt = $this->pdo->prepare(
            'UPDATE todo_items SET sort_order = :sort_order WHERE id = :id'
        );
    ...
    {
        $stmt = $this->pdo->prepare('SELECT COUNT(*) FROM todo_items WHERE category_id = :category_id');
        $stmt->execute([
            'category_id' => $categoryId,
        ]);
