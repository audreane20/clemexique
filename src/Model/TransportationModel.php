...
class TransportationModel
{
    ...
    {
        $stmt = $this->pdo->prepare(
            'INSERT INTO transportation_items (category_id, name, address, note, website_url, website_label, sort_order)
             VALUES (:category_id, :name, :address, :note, :website_url, :website_label, :sort_order)'
        );
    ...
    private function deleteItemInLanguage(string $language, int $categoryIndex, int $itemIndex, bool $strict): void
    {
        ...
        $stmt = $this->pdo->prepare('DELETE FROM transportation_items WHERE id = :id');
        $stmt->execute([
            'id' => (int) $itemRow['id'],
        ]);
    ...
    private function deleteCategoryInLanguage(string $language, int $categoryIndex, bool $strict): void
    {
        ...
        $stmt = $this->pdo->prepare('DELETE FROM transportation_categories WHERE id = :id');
        $stmt->execute([
            'id' => (int) $categoryRow['id'],
        ]);
    ...
    {
        $stmt = $this->pdo->prepare(
            'UPDATE transportation_items SET sort_order = :sort_order WHERE id = :id'
        );
