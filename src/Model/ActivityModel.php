...
class ActivityModel
{
    ...
    {
        $media = $this->normalizeMediaPayload($data['media'] ?? []);
        $payload = [
    ...
    private function normalizeMediaPayload(mixed $media): array
    {
        if (!is_array($media)) {
            return [];
        }
    ...
    private function resolveDestinationCategoryId(string $language, array $data): int
    {
        ...
        if ($categoryChoice === '__new__') {
            ...
            $stmt = $this->pdo->prepare(
                'INSERT INTO activity_categories (language_code, title, icon_code, sort_order)
    ...
    {
        $stmt = $this->pdo->prepare(
            'INSERT INTO activities (category_id, name, address, description, website_url, website_label, media_json, sort_order)
             VALUES (:category_id, :name, :address, :description, :website_url, :website_label, :media_json, :sort_order)'
        );
    ...
    {
        $translatedData = $data;
        $translatedData['description'] = $this->translateText((string) ($data['description'] ?? ''), $sourceLanguage, $targetLanguage);
    ...
    private function deleteCategoryInLanguage(string $language, int $categoryIndex, bool $strict): void
    {
        ...
        $stmt = $this->pdo->prepare('DELETE FROM activity_categories WHERE id = :id');
        $stmt->execute([
            'id' => (int) $categoryRow['id'],
        ]);
    ...
    {
        $stmt = $this->pdo->prepare(
            'SELECT * FROM activities WHERE category_id = :category_id ORDER BY sort_order ASC, id ASC'
        );
        $stmt->execute([
            'category_id' => $categoryId,
        ]);
    ...
    {
        $stmt = $this->pdo->prepare(
            'UPDATE activity_categories SET sort_order = :sort_order WHERE id = :id'
        );
    ...
    {
        $stmt = $this->pdo->prepare(
            'UPDATE activities SET sort_order = :sort_order WHERE id = :id'
        );
    ...
    {
        $stmt = $this->pdo->prepare('SELECT COUNT(*) FROM activities WHERE category_id = :category_id');
        $stmt->execute([
            'category_id' => $categoryId,
        ]);
    ...
    private function mapItemRowToViewData(array $itemRow): array
    {
        ...
        $media = json_decode((string) ($itemRow['media_json'] ?? ''), true);
        ...
        return [
            ...
            'media' => $this->normalizeMediaPayload($media),
        ];
    }
    ...
    private function localizedMessage(string $key, string $language): string
    {
        $messages = [
            ...
            'new_category_title_required' => [
                ...
                'fr' => 'Veuillez saisir le titre de la nouvelle categorie.',
                'es' => 'Ingresa el titulo de la nueva categoria.',
            ],
            'select_category' => [
                'en' => 'Please select a category.',
                'fr' => 'Veuillez selectionner une categorie.',
                'es' => 'Selecciona una categoria.',
            ],
            'item_not_found' => [
                'en' => 'Activity not found.',
                'fr' => 'Activite introuvable.',
                'es' => 'Actividad no encontrada.',
            ],
            'category_not_found' => [
                'en' => 'Category not found.',
                'fr' => 'Categorie introuvable.',
